...
get_token_at_helper(Tokens, Target_index, Current_index) ->
    case Tokens of
        ...
        [First | Rest] when Current_index =:= Target_index ->
            {ok, First};

        [_ | Rest@1] ->
            get_token_at_helper(Rest@1, Target_index, Current_index + 1)
    end.

-file("src/geql/parser.gleam", 244).
...
-file("src/geql/parser.gleam", 212).
...
-file("src/geql/parser.gleam", 224).
-spec parse_name(parser()) -> {ok, binary()} | {error, parse_error()}.
parse_name(Parser) ->
    case consume_token(Parser) of
        {ok, {{token_with_position, {name, Name}, _}, _}} ->
            {ok, Name};
...
-file("src/geql/parser.gleam", 263).
...
-file("src/geql/parser.gleam", 279).
-spec is_operation_type(geql@lexer:token()) -> boolean().
is_operation_type(Token) ->
    case Token of
        'query' ->
            true;
    ...
    gleam@result:'try'(
        ...
        fun(_use0) ->
            ...
            case peek_token(Parser@1) of
                        ...
                        fun(_use0@1) ->
                            ...
                            gleam@result:'try'(
                                ...
                                fun(_use0@2) ->
                                    ...
                                    {Selection_set, Parser@4} = case peek_token(
                                        Parser@3
                                    ) of
                                        {ok,
                                            {token_with_position, left_brace, _}} ->
                                            case parse_selection_set(Parser@3) of
                                                {ok, {Ss, P}} ->
                                                    {{some, Ss}, P};
                ...
                _ ->
                    {Selection_set@1, Parser@5} = case peek_token(Parser@1) of
                        {ok, {token_with_position, left_brace, _}} ->
                            case parse_selection_set(Parser@1) of
                                {ok, {Ss@1, P@1}} ->
                                    {{some, Ss@1}, P@1};
