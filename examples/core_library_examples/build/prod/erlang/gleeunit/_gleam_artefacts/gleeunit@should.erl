...
-spec be_ok({ok, FMF} | {error, any()}) -> FMF.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, FMK}) -> FMK.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(FMN)) -> FMN.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
