...
-file("src/geql/schema.gleam", 145).
...
-file("src/geql/schema.gleam", 157).
...
-file("src/geql/schema.gleam", 168).
...
-file("src/geql/schema.gleam", 180).
-spec schema() -> schema().
schema() ->
    {schema, none, none, none, maps:new()}.

-file("src/geql/schema.gleam", 184).
...
-file("src/geql/schema.gleam", 188).
...
-file("src/geql/schema.gleam", 192).
...
-file("src/geql/schema.gleam", 196).
...
-file("src/geql/schema.gleam", 210).
-spec object(binary()) -> object_type().
object(Name) ->
    {object_type, Name, none, maps:new(), []}.

-file("src/geql/schema.gleam", 214).
...
-file("src/geql/schema.gleam", 218).
...
-file("src/geql/schema.gleam", 222).
...
-file("src/geql/schema.gleam", 227).
...
-file("src/geql/schema.gleam", 237).
...
-file("src/geql/schema.gleam", 244).
...
-file("src/geql/schema.gleam", 254).
-spec resolver(
    field_definition(),
...
-file("src/geql/schema.gleam", 275).
...
-file("src/geql/schema.gleam", 283).
-spec string_type() -> field_type().
string_type() ->
    {named, <<"String"/utf8>>}.

-file("src/geql/schema.gleam", 287).
-spec int_type() -> field_type().
int_type() ->
    {named, <<"Int"/utf8>>}.

-file("src/geql/schema.gleam", 291).
-spec float_type() -> field_type().
float_type() ->
    {named, <<"Float"/utf8>>}.

-file("src/geql/schema.gleam", 295).
-spec boolean_type() -> field_type().
boolean_type() ->
    {named, <<"Boolean"/utf8>>}.

-file("src/geql/schema.gleam", 299).
-spec id_type() -> field_type().
id_type() ->
    {named, <<"ID"/utf8>>}.

-file("src/geql/schema.gleam", 303).
-spec named_type(binary()) -> field_type().
named_type(Name) ->
    {named, Name}.

-file("src/geql/schema.gleam", 307).
-spec non_null(field_type()) -> field_type().
non_null(Inner) ->
    {non_null, Inner}.

-file("src/geql/schema.gleam", 311).
-spec list_type(field_type()) -> field_type().
list_type(Inner) ->
    {list, Inner}.

-file("src/geql/schema.gleam", 316).
...
-file("src/geql/schema.gleam", 326).
...
-file("src/geql/schema.gleam", 330).
-spec serialize(
    scalar_type(),
    fun((gleam@dynamic:dynamic_()) -> {ok, gleam@dynamic:dynamic_()} |
        {error, binary()})
...
-file("src/geql/schema.gleam", 337).
-spec parse_value(
    scalar_type(),
    fun((gleam@dynamic:dynamic_()) -> {ok, gleam@dynamic:dynamic_()} |
        {error, binary()})
...
-file("src/geql/schema.gleam", 344).
-spec parse_literal(
    scalar_type(),
    fun((gleam@dynamic:dynamic_()) -> {ok, gleam@dynamic:dynamic_()} |
        {error, binary()})
...
-file("src/geql/schema.gleam", 352).
...
-file("src/geql/schema.gleam", 357).
...
-file("src/geql/schema.gleam", 364).
...
-file("src/geql/schema.gleam", 371).
...
-file("src/geql/schema.gleam", 376).
