-module(geql@schema_gen).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch]).
-define(FILEPATH, "src/geql/schema_gen.gleam").
...
add_fields_to_object(Object, Fields) ->
    case Fields of
        ...
        [Field | Rest] ->
            _pipe = Object,
            _pipe@1 = geql@schema:field(_pipe, Field),
            add_fields_to_object(_pipe@1, Rest)
    end.

-file("src/geql/schema_gen.gleam", 177).
...
-file("src/geql/schema_gen.gleam", 168).
...
-file("src/geql/schema_gen.gleam", 71).
...
-file("src/geql/schema_gen.gleam", 183).
...
-file("src/geql/schema_gen.gleam", 116).
-spec create_fields_from_specs(list(field_spec())) -> list(geql@schema:field_definition()).
create_fields_from_specs(Field_specs) ->
    case Field_specs of
        ...
        [Spec | Rest] ->
            [begin
                    _pipe = geql@schema:field_def(
                        erlang:element(2, Spec),
                        erlang:element(3, Spec)
                    ),
                    _pipe@1 = geql@schema:field_description(
                        _pipe,
...
-file("src/geql/schema_gen.gleam", 189).
...
-file("src/geql/schema_gen.gleam", 198).
...
-file("src/geql/schema_gen.gleam", 207).
...
-file("src/geql/schema_gen.gleam", 145).
        ...
        geql@schema:field(
            ...
            begin
                _pipe@2 = geql@schema:field_def(
                    ...
                    geql@schema:named_type(Type_name)
                ),
                _pipe@3 = geql@schema:field_description(
                    _pipe@2,
                    <<"Get a "/utf8, Type_name/binary>>
                ),
                geql@schema:resolver(_pipe@3, Root_resolver)
            end
        )
    end,
    _pipe@4 = geql@schema:schema(),
    _pipe@5 = geql@schema:'query'(_pipe@4, Query_type),
    geql@schema:add_type(_pipe@5, {object_type_def, Object_type}).

-file("src/geql/schema_gen.gleam", 216).
