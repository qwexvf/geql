-module(geql@sdl_demo).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch]).
-define(FILEPATH, "src/geql/sdl_demo.gleam").
...
-file("src/geql/sdl_demo.gleam", 259).
...
-file("src/geql/sdl_demo.gleam", 249).
-spec count_definitions(list(geql@sdl_ast:type_system_definition()), integer()) -> binary().
...
-file("src/geql/sdl_demo.gleam", 300).
-spec get_type_kind(geql@sdl_ast:type_def()) -> binary().
...
-file("src/geql/sdl_demo.gleam", 288).
-spec print_definition(geql@sdl_ast:type_system_definition(), integer()) -> nil.
print_definition(Definition, Index) ->
    Index_str = int_to_string(Index),
    case Definition of
        {type_definition, Type_def} ->
            Type_name = geql@sdl_ast:get_type_name(Type_def),
...
-file("src/geql/sdl_demo.gleam", 275).
...
-file("src/geql/sdl_demo.gleam", 236).
-spec print_document_summary(geql@sdl_ast:s_d_l_document()) -> nil.
...
-file("src/geql/sdl_demo.gleam", 341).
-spec format_lex_error(geql@sdl_lexer:s_d_l_lexer_error()) -> binary().
...
-file("src/geql/sdl_demo.gleam", 350).
-spec format_token(geql@sdl_lexer:s_d_l_token()) -> binary().
format_token(Token) ->
    case Token of
        {name, Name} ->
...
-file("src/geql/sdl_demo.gleam", 311).
-spec print_parse_error(geql@sdl_parser:s_d_l_parse_error()) -> nil.
...
-file("src/geql/sdl_demo.gleam", 33).
...
demo_basic_types() ->
  ...
  "/utf8>>,
    ...
    case geql@sdl_parser:parse_sdl(Schema_sdl) of
        {ok, Document} ->
...
-file("src/geql/sdl_demo.gleam", 69).
...
-file("src/geql/sdl_demo.gleam", 106).
...
-file("src/geql/sdl_demo.gleam", 134).
...
-file("src/geql/sdl_demo.gleam", 170).
...
-file("src/geql/sdl_demo.gleam", 207).
...
demo_error_handling() ->
  ...
  "/utf8>>,
    ...
    case geql@sdl_parser:parse_sdl(Invalid_sdl) of
        {ok, _} ->
...
-file("src/geql/sdl_demo.gleam", 10).
