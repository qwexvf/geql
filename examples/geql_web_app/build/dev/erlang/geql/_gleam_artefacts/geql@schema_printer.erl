-module(geql@schema_printer).
-compile([no_auto_import, nowarn_unused_vars, nowarn_unused_function, nowarn_nomatch]).
-define(FILEPATH, "src/geql/schema_printer.gleam").
...
-file("src/geql/schema_printer.gleam", 234).
?DOC(" Print field type (handles NonNull, List, Named)\n").
-spec print_field_type(geql@schema:field_type()) -> binary().
...
-file("src/geql/schema_printer.gleam", 210).
?DOC(" Print argument definition\n").
-spec print_argument_definition(geql@schema:argument_definition()) -> binary().
...
-file("src/geql/schema_printer.gleam", 243).
?DOC(" Print schema definition if it has custom root types\n").
-spec print_schema_definition(geql@schema:schema()) -> binary().
...
-file("src/geql/schema_printer.gleam", 274).
?DOC(" Check if schema has custom root type names\n").
-spec has_custom_root_types(geql@schema:schema()) -> boolean().
...
-file("src/geql/schema_printer.gleam", 122).
?DOC(" Print union type: union SearchResult = User | Post\n").
-spec print_union_type(geql@schema:union_type()) -> binary().
...
-file("src/geql/schema_printer.gleam", 299).
?DOC(" Print description inline\n").
-spec print_description_inline(binary()) -> binary().
print_description_inline(Desc) ->
    <<"# "/utf8, Desc/binary>>.

-file("src/geql/schema_printer.gleam", 161).
?DOC(" Print field definition within a type\n").
-spec print_field_definition(geql@schema:field_definition(), binary()) -> binary().
...
-file("src/geql/schema_printer.gleam", 45).
?DOC(" Print object type: type User { ... }\n").
-spec print_object_type(geql@schema:object_type()) -> binary().
...
-file("src/geql/schema_printer.gleam", 102).
?DOC(" Print interface type: interface Node { ... }\n").
-spec print_interface_type(geql@schema:interface_type()) -> binary().
...
-file("src/geql/schema_printer.gleam", 190).
?DOC(" Print input field definition\n").
-spec print_input_field_definition(
    geql@schema:input_field_definition(),
    binary()
) -> binary().
...
-file("src/geql/schema_printer.gleam", 141).
?DOC(" Print input object type: input CreateUserInput { ... }\n").
-spec print_input_object_type(geql@schema:input_object_type()) -> binary().
...
-file("src/geql/schema_printer.gleam", 221).
?DOC(" Print enum value\n").
-spec print_enum_value(geql@schema:enum_value_definition(), binary()) -> binary().
...
-file("src/geql/schema_printer.gleam", 82).
?DOC(" Print enum type: enum Status { ACTIVE INACTIVE }\n").
-spec print_enum_type(geql@schema:enum_type()) -> binary().
...
-file("src/geql/schema_printer.gleam", 33).
?DOC(" Print individual type definition\n").
-spec print_type_definition(geql@schema:type_definition()) -> binary().
...
-file("src/geql/schema_printer.gleam", 12).
?DOC(" Print a schema to SDL format\n").
-spec print_schema(geql@schema:schema()) -> binary().
...
-file("src/geql/schema_printer.gleam", 304).
...
demo_schema_printing() ->
    ...
    Post_type = begin
        _pipe@7 = geql@schema:object(<<"Post"/utf8>>),
        _pipe@8 = geql@schema:description(_pipe@7, <<"A blog post"/utf8>>),
        _pipe@9 = geql@schema:field(
            _pipe@8,
    ...
    Query_type = begin
        _pipe@11 = geql@schema:object(<<"Query"/utf8>>),
        _pipe@13 = geql@schema:field(
            _pipe@11,
            begin
    ...
    Complete_schema = begin
        ...
        geql@schema:add_type(_pipe@16, {object_type_def, Post_type})
    end,
