...
-opaque trie(DTU, DTV) :: {trie,
        gleam@option:option(DTV),
        gleam@dict:dict(DTU, trie(DTU, DTV))}.
...
-spec do_delete(trie(DUD, DUE), list(DUD)) -> gleam@option:option(trie(DUD, DUE)).
do_delete(Trie, Path) ->
    case {Path, Trie} of
        {[], {trie, _, Children_map}} ->
...
-spec fold(trie(DUL, DUM), DUP, fun((DUP, list(DUL), DUM) -> DUP)) -> DUP.
...
-spec get(trie(DUX, DUY), list(DUX)) -> {ok, DUY} | {error, nil}.
get(From, Path) ->
    case {Path, From} of
        {[], {trie, none, _}} ->
            {error, nil};
...
-spec has_path(trie(DVE, any()), list(DVE)) -> boolean().
has_path(Trie, Path) ->
...
-spec map(trie(DVU, DVV), fun((DVV) -> DVY)) -> trie(DVU, DVY).
map(Trie, Fun) ->
    {trie,
...
-spec delete(trie(DTW, DTX), list(DTW)) -> trie(DTW, DTX).
delete(Trie, Path) ->
...
-spec insert(trie(DVJ, DVK), list(DVJ), DVK) -> trie(DVJ, DVK).
...
-spec from_list(list({list(DUR), DUT})) -> trie(DUR, DUT).
from_list(List) ->
    gleam@list:fold(
        List,
        new(),
...
-spec paths(trie(DWF, any())) -> list(list(DWF)).
paths(Trie) ->
...
-spec singleton(list(DWL), DWN) -> trie(DWL, DWN).
...
-spec subtrie(trie(DWU, DWV), list(DWU)) -> {ok, trie(DWU, DWV)} | {error, nil}.
subtrie(Trie, Prefix) ->
    case {Prefix, Trie} of
        {[], _} ->
            {ok, Trie};
...
-spec to_list(trie(DXD, DXE)) -> list({list(DXD), DXE}).
to_list(Trie) ->
...
-spec do_update(
    trie(DXS, DXT),
    list(DXS),
    fun((gleam@option:option(DXT)) -> gleam@option:option(DXT))
) -> gleam@option:option(trie(DXS, DXT)).
...
-spec update(
    trie(DXJ, DXK),
    list(DXJ),
    fun((gleam@option:option(DXK)) -> gleam@option:option(DXK))
) -> trie(DXJ, DXK).
...
-spec values(trie(any(), DYD)) -> list(DYD).
values(Trie) ->
