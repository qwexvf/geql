...
-file("src/birdie/internal/titles.gleam", 361).
?DOC(false).
...
-file("src/birdie/internal/titles.gleam", 386).
?DOC(false).
...
snap_call(Birdie_import, Expression) ->
    case Expression of
        {call,
            _,
            Function,
            [{unlabelled_field, Title}, {labelled_field, <<"content"/utf8>>, _}]} ->
        ...
        {call,
            _,
            Function,
            [{unlabelled_field, _}, {labelled_field, <<"title"/utf8>>, Title}]} ->
            Is_snap_function = is_snap_function(Function, Birdie_import),
            gleam@bool:guard(
                not Is_snap_function,
                {error, nil},
        ...
        {call,
            _,
            Function,
            [{labelled_field, <<"content"/utf8>>, _}, {unlabelled_field, Title}]} ->
            Is_snap_function = is_snap_function(Function, Birdie_import),
            gleam@bool:guard(
                not Is_snap_function,
                {error, nil},
        ...
        {call,
            _,
            Function,
            [{labelled_field, <<"content"/utf8>>, _},
                {labelled_field, <<"title"/utf8>>, Title}]} ->
            Is_snap_function = is_snap_function(Function, Birdie_import),
            gleam@bool:guard(
                not Is_snap_function,
                {error, nil},
        ...
        {call, _, Function, [{labelled_field, <<"title"/utf8>>, Title}, _]} ->
            Is_snap_function = is_snap_function(Function, Birdie_import),
            gleam@bool:guard(
                not Is_snap_function,
                {error, nil},
        ...
        {binary_operator,
            ...
            pipe,
            Title,
            {call, _, Function, [{labelled_field, <<"content"/utf8>>, _}]}} ->
            Is_snap_function = is_snap_function(Function, Birdie_import),
            gleam@bool:guard(
                not Is_snap_function,
                {error, nil},
        ...
        {binary_operator,
            ...
            pipe,
            _,
            {call, _, Function, [{labelled_field, <<"title"/utf8>>, Title}]}} ->
            Is_snap_function = is_snap_function(Function, Birdie_import),
            gleam@bool:guard(
                not Is_snap_function,
                {error, nil},
...
?DOC(false).
-spec try_or({ok, INT} | {error, any()}, INX, fun((INT) -> INX)) -> INX.
...
-file("src/birdie/internal/titles.gleam", 430).
?DOC(false).
-spec try_fold_expression(
    glance:expression(),
    IMR,
    fun((IMR, glance:expression()) -> {ok, IMR} | {error, IMS})
) -> {ok, IMR} | {error, IMS}.
...
-spec try_fold_statements(
    ...
    IML,
    fun((IML, glance:expression()) -> {ok, IML} | {error, IMM})
) -> {ok, IML} | {error, IMM}.
...
-file("src/birdie/internal/titles.gleam", 523).
?DOC(false).
-spec try_fold_fields(
    list(glance:field(glance:expression())),
    IMZ,
    fun((IMZ, glance:expression()) -> {ok, IMZ} | {error, INA})
) -> {ok, IMZ} | {error, INA}.
...
-spec try_fold_clauses(
    list(glance:clause()),
    ING,
    fun((ING, glance:expression()) -> {ok, ING} | {error, INH})
) -> {ok, ING} | {error, INH}.
...
-spec try_fold_expressions(
    ...
    INN,
    fun((INN, glance:expression()) -> {ok, INN} | {error, INO})
) -> {ok, INN} | {error, INO}.
