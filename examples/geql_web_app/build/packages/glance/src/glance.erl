...
-type definition(HVX) :: {definition, list(attribute()), HVX}.
...
-type bit_string_segment_option(HVY) :: bytes_option |
    int_option |
    float_option |
    bits_option |
    utf8_option |
...
-type record_update_field(HVZ) :: {record_update_field,
        binary(),
        gleam@option:option(HVZ)}.

-type variant_field() :: {labelled_variant_field, type(), binary()} |
    {unlabelled_variant_field, type()}.

-type field(HWA) :: {labelled_field, binary(), HWA} |
    {shorthand_field, binary()} |
    {unlabelled_field, HWA}.
...
-type parsed_list(HWB) :: {parsed_list,
        list(HWB),
        gleam@option:option(HWB),
        list({glexer@token:token(), glexer:position()}),
        integer()}.
...
-spec expect_name(
    list({glexer@token:token(), glexer:position()}),
    fun((binary(), list({glexer@token:token(), glexer:position()})) -> {ok, HWT} |
        {error, error()})
) -> {ok, HWT} | {error, error()}.
...
-spec until(
    ...
    HWY,
    list({glexer@token:token(), glexer:position()}),
    fun((HWY, list({glexer@token:token(), glexer:position()})) -> {ok,
            {HWY, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {HWY, integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec list(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {ICR, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    gleam@option:option(fun((span()) -> ICR)),
    list(ICR),
    list({glexer@token:token(), glexer:position()})
) -> {ok, parsed_list(ICR)} | {error, error()}.
...
-spec bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {IAO, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list(bit_string_segment_option(IAO)),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(IAO)),
            list({glexer@token:token(), glexer:position()})}} |
...
-spec optional_bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {IAH, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(IAH)),
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec bit_string_segment(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {IAA, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {{IAA, list(bit_string_segment_option(IAA))},
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec delimited(
    ...
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {ICH, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok, {list(ICH), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec comma_delimited(
    ...
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {IDJ, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok,
        {list(IDJ), integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-spec field(
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {IEU, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {field(IEU), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
field(Tokens, Parser) ->
    case Tokens of
