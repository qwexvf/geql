...
pub fn print_schema(schema_def: schema.Schema) -> String {
  ...
  let type_parts = dict.to_list(schema_def.types)
...
fn print_object_type(obj: schema.ObjectType) -> String {
  ...
  let header = case list.length(obj.interfaces) {
    0 -> header
    _ -> header <> " implements " <> string.join(list.map(obj.interfaces, fn(i) { i.name }), " & ")
  }
  
  let fields = dict.to_list(obj.fields)
...
fn print_enum_type(enum: schema.EnumType) -> String {
  ...
  let values = dict.to_list(enum.values)
...
fn print_interface_type(interface: schema.InterfaceType) -> String {
  ...
  let fields = dict.to_list(interface.fields)
    |> list.map(fn(entry) { print_field_definition(entry.1, "  ") })
    |> string.join("\n")
...
fn print_union_type(union: schema.UnionType) -> String {
  ...
  let members = list.map(union.types, fn(t) { t.name })
    |> string.join(" | ")
...
fn print_input_object_type(input: schema.InputObjectType) -> String {
  ...
  let fields = dict.to_list(input.fields)
...
fn print_field_definition(field: schema.FieldDefinition, indent: String) -> String {
  ...
  let base = case dict.size(field.arguments) {
    0 -> base
    _ -> {
      let args = dict.to_list(field.arguments)
...
fn print_input_field_definition(field: schema.InputFieldDefinition, indent: String) -> String {
  let base = indent <> field.name <> ": " <> print_field_type(field.field_type)
  
  // Add default value if any
  let base = case field.default_value {
    Some(_) -> base <> " = " <> "defaultValue" // TODO: Proper value printing
    None -> base
  }
...
/// Print enum value
fn print_enum_value(enum_value: schema.EnumValueDefinition, indent: String) -> String {
...
fn print_schema_definition(schema_def: schema.Schema) -> String {
  ...
  let parts = case schema_def.mutation {
    Some(mutation) if mutation.name != "Mutation" -> ["mutation: " <> mutation.name, ..parts]
    _ -> parts
  }
  
  let parts = case schema_def.subscription {
    Some(subscription) if subscription.name != "Subscription" -> ["subscription: " <> subscription.name, ..parts]
    _ -> parts
  }
...
pub fn demo_schema_printing() -> Nil {
  ...
  let query_type = 
    ...
    |> schema.field(
      schema.field_def("posts", schema.list_type(schema.non_null(schema.named_type("Post"))))
    )
