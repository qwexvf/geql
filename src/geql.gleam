...
pub fn main() -> Nil {
  ...
  io.println("")
  io.println("GeQL is a pure GraphQL library for Gleam with zero dependencies beyond gleam_stdlib")
  io.println("")
  ...
  case result.data {
    ...
    None -> {
      io.println("ℹ️  Query execution shows where Dynamic serialization would be needed")
      io.println("   (Use gleam_json or external functions in real implementations)")
    }
  }
  ...
  let user_type = 
    ...
    |> schema.field(
      ...
      |> schema.resolver(fn(_info) { 
        Error("Demo: Would return user ID here")
      })
    )
    |> schema.field(
